...
lib_rb = 'module Lib
  def ar_init
      ...
      ActiveRecord::Base.establish_connection((defined?(production) && production?) ? config["production"] : config["development"])
  ...
  helpers do
    ...
    def authorized?
      ...
      @auth.provided? && @auth.basic? && @auth.credentials && @auth.credentials == [\'admin\', \'qwsa\']
    end

  end

  def version
    "Running on Sinatra #{Sinatra::VERSION} (Generated by <a href="http://github.com/etozzato/SinaTree">SinaTree</a>) <br /><br /> <script language=\"javascript\" src=\"http://app.headcounters.com/digits.js\"></script>"
  end
...
unless ARGV[0]
  ...
  unless File.exists?(name)
    ...
    `cd #{name} && echo '#{app_rb}' >> app.rb && echo '#{config_ru}' >> config.ru && echo '#{rakefile.gsub('APP_NAME', name + '_db')}' >> Rakefile && echo '#{lib_rb}' >> lib.rb && echo '#{models_rb}' >> models/models.rb && echo '#{database_yml.gsub('APP_NAME', name + '_db')}' >> config/database.yml`
